    ...
    {cartItems.length === 0 ?(
      <div className={CartCSS.EmptyCartContainer}>
        <div className={CartCSS.imagecontainer}>
          <img src={EmptyCart} className={CartCSS.emptycart} alt='Empty Cart'/>
        </div>
        <h1 align='center' style={{color:'#2D6A4F'}}>Oops! Your cart is empty, let's go shopping!</h1>
      </div>
    ) : (
    <div className={CartCSS.Container}>
      ...
      <div key={product.id} className={CartCSS.card}>
        ...
        <div className={CartCSS.details}>
          ...
          <p className={CartCSS.price}><b>₹{product.price}.00</b></p>
          <p className={CartCSS.instock}>In Stock</p>
          <p className={CartCSS.emi}>EMI options are available on selected banks.</p>

          <div className={CartCSS.quantityControls}>
            <div className={CartCSS.quantityBack}>
              <button className={CartCSS.quantityBtn} onClick={() => handleDecrement(product.id)}>-</button>
              <span className={CartCSS.quantity}>{product.quantity}</span>
              <button className={CartCSS.quantityBtn} onClick={() => handleIncrement(product.id)}>+</button>
            </div>
          </div>

          <button className={CartCSS.removeButton} onClick={() => handleRemove(product.id)}>
            Remove
          </button>
      ...
      <div className={CartCSS.amountContainer}>
        <h2 className={CartCSS.subamount}>Sub Total: ₹{getSubTotalAmount()}</h2>
        {shippingAmount === 0 ? (
          <p className={CartCSS.shipping}>Shipping Charges: Free Shipping Charges</p>
        ) : (
          <p className={CartCSS.shipping}>Shipping Charges: +₹{shippingAmount}</p>
        )}
        <p className={CartCSS.tax}>Tax: +₹0</p>
        <span className={CartCSS.amountline}></span>
        <h1 className={CartCSS.totalamount}>Total Amount: ₹{getTotalAmount()}</h1>
        {ProceedToCheckout &&
        <button className={CartCSS.checkout} onClick={handleProceedToCheck}>
          Proceed to Checkout
        </button>}
