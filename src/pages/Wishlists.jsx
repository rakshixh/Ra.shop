...
function Wishlists() {
  ...
  const wishlistProducts = ProductData.filter((product) => wishlist.includes(product.id));
    ...
    {wishlistProducts.length === 0 ? (
      <div className={HomeCSS.EmptyWishlistContainer}>
        <div className={HomeCSS.imagecontainer}>
          <img src={WishlistEmpty} className={HomeCSS.emptywishlistimage} alt='Empty Cart'/>
        </div>
        <h1 align='center' style={{color:'#2D6A4F'}}>Create your wishlist and save your favorite products!</h1>
      </div>
    ) : (
